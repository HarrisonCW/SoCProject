...
void gsensor_check(SpiCore *spi_p, GpoCore *led_p, float *coord) {
...
int main() {
   ...
   float linX, angX, angZ;
   float angX, angY, angZ;

   while (1) {
	   gsensor_check(*spi_p,*led_p, coord);

	   //Converting linear acceleration to angular
	   linX = coord[0];
	   linY = coord[1];
	   linZ = coord[2];

	   angX = atan(linX / sqrt(square(linY) + square(linZ)));
	   angY = atan(linY / sqrt(square(linX) + square(linZ)));
	   angZ = atan(sqrt(square(linX) + square(linY)));
